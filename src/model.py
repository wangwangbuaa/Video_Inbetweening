...
from RBMCell import RBM, ImageEncoder
from ops import *
from utils import *
...
class Encoder(object):
    ...
    def image_encoder(self, xt, reuse):
        # xt: H0 x W0 x {1, 3}
        L1 = conv2d(xt, output_dim=64, k_h=4, k_w=4, d_h=2, d_w=2,
                    name='image_conv1', reuse=reuse)
        L2 = conv2d(L1, output_dim=64, k_h=3, k_w=3, d_h=1, d_w=1,
                    name='image_conv2', reuse=reuse)
        L3 = conv2d(L2, output_dim=128, k_h=4, k_w=4, d_h=2, d_w=2,
                    name='image_conv3', reuse=reuse)
        L4 = conv2d(L3, output_dim=128, k_h=3, k_w=3, d_h=1, d_w=1,
                    name='image_conv4', reuse=reuse)
        L5 = conv2d(L4, output_dim=256, k_h=4, k_w=4, d_h=2, d_w=2,
                    name='image_conv5', reuse=reuse)
        L6 = conv2d(L5, output_dim=256, k_h=3, k_w=3, d_h=1, d_w=1,
                    name='image_conv6', reuse=reuse)
        L7 = conv2d(L6, output_dim=64, k_h=3, k_w=3, d_h=1, d_w=1,
                    name='image_conv7', reuse=reuse)
        return L7  # H0/8 × W0/8 × 64

    def video_generator(self, xt, reuse, dim_c):
        # xt 16 × H0/8 × W0/8 × 64
        L1 = transConv3d(xt, output_dim=256, k_h=3, k_w=3, k_dep=3, d_h=1,
                         d_w=1, d_dep=1,
                         name='video_transconv3d1', reuse=reuse)
        L2 = transConv3d(L1, output_dim=256, k_h=3, k_w=3, k_dep=3, d_h=1,
                         d_w=1, d_dep=1,
                         name='video_transconv3d2', reuse=reuse)
        L3 = transConv3d(L2, output_dim=128, k_h=4, k_w=4, k_dep=3, d_h=2,
                         d_w=2, d_dep=1,
                         name='video_transconv3d3', reuse=reuse)
        L4 = transConv3d(L3, output_dim=128, k_h=3, k_w=3, k_dep=3, d_h=1,
                         d_w=1, d_dep=1,
                         name='video_transconv3d4', reuse=reuse)
        L5 = transConv3d(L4, output_dim=64, k_h=4, k_w=4, k_dep=3, d_h=2, d_w=2,
                         d_dep=1,
                         name='video_transconv3d5', reuse=reuse)
        L6 = transConv3d(L5, output_dim=64, k_h=3, k_w=3, k_dep=3, d_h=1, d_w=1,
                         d_dep=1,
                         name='video_transconv3d6', reuse=reuse)
        L7 = transConv3d(L6, output_dim=dim_c, k_h=4, k_w=4, k_dep=3, d_h=2,
                         d_w=2, d_dep=1,
                         name='video_transconv3d7', reuse=reuse)
        return L7  # 16 × H0 × W0 × {1, 3}

    def video_discriminator(self, xt, reuse):
        # 16 × H0 × W0 × {1, 3}
        L1 = conv3d(xt, output_dim=64, k_h=4, k_w=4, k_dep=4, d_h=2,
                    d_w=2, d_dep=1,
                    name='video_conv3d1', reuse=reuse)
        L2 = conv3d(L1, output_dim=128, k_h=4, k_w=4, k_dep=4, d_h=2,
                    d_w=2, d_dep=1,
                    name='video_conv3d2', reuse=reuse)
        L3 = conv3d(L2, output_dim=256, k_h=4, k_w=4, k_dep=4, d_h=2,
                    d_w=2, d_dep=1,
                    name='video_conv3d3', reuse=reuse)
        L4 = conv3d(L3, output_dim=512, k_h=4, k_w=4, k_dep=4, d_h=2,
                    d_w=2, d_dep=1,
                    name='video_conv3d4', reuse=reuse)

        h = tf.sigmoid(
            linear(tf.reshape(L4, [L4.shape[0], -1]), 1, 'dis_video'))

        return h

    def short_cut(self, xt, C, reuse, name):
        l1 = AvgPooling(xt, [2, 2], [2, 2])
        l2 = conv2d(l1, C, k_h=1, k_w=1, d_h=1, d_w=1, name=name, reuse=reuse)
        return l2

    def image_discriminator(self, xt, reuse, dim_c):
        # xt H0 × W0 × {1, 3}
        L1 = conv2d(xt, dim_c, k_h=3, k_w=3,
                    d_h=1, d_w=1, name='image_disconv1', reuse=reuse)
        L2 = conv2d(L1, 64, k_h=4, k_w=4,
                    d_h=2, d_w=2, name='image_disconv2',
                    reuse=reuse) + self.short_cut(xt, 64, reuse,
                                                  name="shortcut1")
        L3 = conv2d(L2, 64, k_h=3, k_w=3,
                    d_h=1, d_w=1, name='image_disconv3', reuse=reuse)
        L4 = conv2d(L3, 128, k_h=4, k_w=4, d_h=2, d_w=2, name='image_disconv4',
                    reuse=reuse) + self.short_cut(xt, 128, reuse,
                                                  name="shortcut2")

        L5 = conv2d(L4, 128, k_h=3, k_w=3, d_h=1, d_w=1, name='image_disconv5',
                    reuse=reuse)
        L6 = conv2d(L5, 256, k_h=4, k_w=4, d_h=2, d_w=2, name='image_disconv6',
                    reuse=reuse) + self.short_cut(xt, 256, reuse,
                                                  name="shortcut3")
        L7 = conv2d(L6, 256, k_h=3, k_w=3, d_h=1, d_w=1, name='image_disconv7',
                    reuse=reuse)
        L8 = conv2d(L7, 512, k_h=4, k_w=4, d_h=2, d_w=2, name='image_disconv8',
                    reuse=reuse) + self.short_cut(xt, 512, reuse,
                                                  name="shortcut4")
        h = tf.sigmoid(
            linear(tf.reshape(L8, [L8.shape[0], -1]), 1, 'dis_image'))
        return h

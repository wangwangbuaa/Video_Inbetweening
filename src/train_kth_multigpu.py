            ...
            for i in range(num_gpus):
                with tf.device('/gpu:%d' % gpu[i]):
                    with tf.name_scope('tower_%d' % gpu[i]) as scope:
                        model.build_model(
                            _diff_in[batch_size * i: batch_size * (i + 1), :, :,
                            :, :],
                        ...
                        def tower_loss_D():
                            ...
                            D_total_loss = tf.add_n(D_losses,
                                                    name='D_total_loss')
                        ...
                        def tower_loss_G():
                            ...
                            G_total_loss = tf.add_n(G_losses,
                                                    name='G_total_loss')
                        ...
                        d_grads = d_optim.compute_gradients(loss_D,
                                                            var_list=model.d_vars)
                        d_tower_grads.append(d_grads)

                        g_grads = g_optim.compute_gradients(loss_G,
                                                            var_list=model.g_vars)
                        g_tower_grads.append(g_grads)

        d_grads = average_gradients(d_tower_grads)
        d_apply_gradient_op = d_optim.apply_gradients(d_grads,
                                                      global_step=global_step)

        g_grads = average_gradients(g_tower_grads)
        g_apply_gradient_op = g_optim.apply_gradients(g_grads,
                                                      global_step=global_step)
                        ...
                        if len(batchidx) == batch_size * num_gpus:
                            ...
                            if updateD:
                                _, summary_str = sess.run(
                                    [d_apply_gradient_op, summary_op],
                                    feed_dict={
                                        _diff_in: diff_batch,
                                        _xt: seq_batch[:, :, :, K - 1],
                                        _target: seq_batch
                            ...
                            if updateG:
                                _, summary_str = sess.run(
                                    [g_apply_gradient_op, summary_op],
                                    feed_dict={
                                        _diff_in: diff_batch,
                                        _xt: seq_batch[:, :, :, K - 1],
                                        _target: seq_batch
                            ...
                            errD_fake, errD_real, errG = sess.run(
                                [errD_fake_op, errD_real_op, errG_op],
                                feed_dict={
                                    _diff_in: diff_batch,
                                    _xt: seq_batch[:, :, :, K - 1],
                                    _target: seq_batch
                            ...
                            if errD_fake < margin or errD_real < margin:
                                updateD = False
                            if errD_fake > (1. - margin) or errD_real > (
                                    1. - margin):
                                updateG = False
